...
begin
  AElement:='TLabel';                  // vstavit' tekst
end;

procedure TForm1.BitBtn2Click(Sender: TObject);
begin
  AElement:='Timage';                 // vstavit' risunok
end;

procedure TForm1.BitBtn3Click(Sender: TObject);
begin
  AElement:='TCheckBox';               // vastavi't variant otveta
end;

procedure TForm1.BitBtn4Click(Sender: TObject);
begin
  UpdateArrays;                                                                         // obonvlenie massiva
  if CurrentTest>0 then CurrentTest:=CurrentTest-1 else ShowMessage('Это первый лист'); // na vopros nazad
  ShowVopr;                                                                             // otobrajenie voprosa
end;

procedure TForm1.BitBtn5Click(Sender: TObject);
begin
  UpdateArrays;                       // obonvlenie massiva
  CurrentTest:=CurrentTest+1;         // na vopros vpered, sozdanie novogo
  ShowVopr;                           // otobrajenie voprosa
end;

procedure TForm1.FormCreate(Sender: TObject);
begin
  Panel3.Top:=0;          //  koordinati
  Panel3.Width:=2;        //  koordinati
  panel3.Visible:=false;  //  vidimost'
  Panel4.Left:=0;         //  koordinati
  Panel4.Height:=2;       //  koordinati
  panel4.Visible:=false;  //  vidimost'
end;

procedure TForm1.LabelClick(Sender: TObject);
begin
  PopupLabel:=(Sender as TLabel).Tag; // prisvoit' funkcii
  PopupMenu1.PopUp;                   // otkrit' popUp menu teksta
end;

procedure TForm1.ImageClick(Sender: TObject);
begin
  PopupImage:=(Sender as TImage).Tag; // prisvoit' funkcii
  PopupMenu2.PopUp;                   // otkrit' popUp menu katinki
end;

procedure TForm1.AnswerClick(Sender: TObject);
begin
  PopupAnswer:=(Sender as TCheckBox).Tag; // prisvoit' funkcii
  PopupMenu3.PopUp;                       // otkrit' popUp menu voprosa
end;
...
begin
  if QuestionDlg ('Внимание','Вы действительно хотите выйти?',mtCustom,[mrYes,'Да', mrNo, 'Нет', 'IsDefault'],'')=mrYes
    then Application.Terminate;  // vihod iz programmi
end;
